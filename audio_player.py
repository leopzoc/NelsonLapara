...
class AudioPlayer:
    ...
    def __init__(self):
        ...
        self._max_volume: float = getattr(cfg, "AUDIO_MAX_VOLUME", 50.0)
        self._update_mixer()
        ...
        log.info(
            "AudioPlayer initialised — default volume: %.0f/%.0f, "
            "session duration: %ds (%.0f min)",
            self._volume, self._max_volume, cfg.AUDIO_SESSION_SEC, cfg.AUDIO_SESSION_SEC / 60,
        )

    def _update_mixer(self) -> None:
        """Apply the 0-50 volume scale to pygame's 0.0-1.0 scale."""
        linear = self._volume / self._max_volume
        self._pygame.mixer.music.set_volume(max(0.0, min(1.0, linear)))
    ...
    def volume_up(self) -> float:
        ...
        with self._lock:
            step = getattr(cfg, "AUDIO_VOLUME_STEP", 5.0)
            self._volume = min(self._max_volume, self._volume + step)
            self._update_mixer()
            log.info("🔊 Volume UP → %.0f/%.0f", self._volume, self._max_volume)
            return self._volume
    ...
    def volume_down(self) -> float:
        ...
        with self._lock:
            ...
            self._update_mixer()
            log.info("🔉 Volume DOWN → %.0f/%.0f", self._volume, self._max_volume)
            return self._volume

    def set_volume(self, level: float) -> None:
        """Set volume to a specific level (0-50)."""
        with self._lock:
            self._volume = max(0.0, min(self._max_volume, level))
            self._update_mixer()
            log.info("🔊 Volume set to %.0f/%.0f", self._volume, self._max_volume)
